...
@Component({
  ...
  styles: [`
    ...
    .connect-banner { display: flex; align-items: center; gap: 1.5rem; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; padding: 2rem; margin-bottom: 2rem; }
    .banner-icon .material-icons { font-size: 48px; color: var(--primary-color); }
    .banner-info { flex: 1; }
    .banner-info h3 { margin: 0; color: var(--text-primary); }
    .banner-info p { margin: 0.3rem 0 0; color: var(--text-secondary); font-size: 0.9rem; }
    .email-layout { display: grid; grid-template-columns: 220px 1fr; gap: 0; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; overflow: hidden; min-height: 600px; }
    .email-sidebar { border-right: 1px solid var(--border-color); padding: 1rem 0; }
    .sidebar-section h4 { padding: 0.5rem 1.2rem; color: var(--text-secondary); font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; margin: 0; }
    .folder { display: flex; align-items: center; gap: 0.6rem; padding: 0.6rem 1.2rem; color: var(--text-secondary); cursor: pointer; transition: all 0.2s; font-size: 0.9rem; }
    .folder:hover { background: rgba(102,126,234,0.1); color: var(--text-primary); }
    .folder.active { background: rgba(102,126,234,0.15); color: var(--primary-color); border-right: 3px solid var(--primary-color); }
    .folder .material-icons { font-size: 20px; }
    .folder .count { margin-left: auto; background: var(--primary-color); color: white; font-size: 0.7rem; padding: 0.1rem 0.5rem; border-radius: 10px; font-weight: 600; }
    .email-main { display: flex; flex-direction: column; }
    .search-bar { display: flex; align-items: center; gap: 0.5rem; padding: 0.8rem 1rem; border-bottom: 1px solid var(--border-color); }
    .search-icon { color: var(--text-secondary); font-size: 20px; }
    .search-bar input { flex: 1; background: transparent; border: none; color: var(--text-primary); font-size: 0.95rem; outline: none; }
    ...
    .email-row { display: flex; align-items: flex-start; gap: 1rem; padding: 1rem 1.2rem; border-bottom: 1px solid var(--border-color); cursor: pointer; transition: background 0.15s; position: relative; }
    ...
    .email-from { color: var(--text-primary); font-size: 0.9rem; }
    .email-from.bold, .email-subject.bold { font-weight: 700; color: var(--text-primary); }
    .email-date { color: var(--text-secondary); font-size: 0.75rem; flex-shrink: 0; }
    .email-subject { color: var(--text-secondary); font-size: 0.9rem; margin-bottom: 0.15rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .email-snippet { color: var(--text-secondary); font-size: 0.8rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .unread-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--primary-color); flex-shrink: 0; margin-top: 0.4rem; }
    ...
    .email-detail { background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; width: 100%; max-width: 700px; max-height: 80vh; overflow-y: auto; padding: 2rem; }
    ...
    .detail-name { color: var(--text-primary); font-weight: 600; }
    .detail-date { color: var(--text-secondary); font-size: 0.8rem; }
    .detail-subject { color: var(--text-primary); font-size: 1.3rem; font-weight: 700; margin-bottom: 1.5rem; }
    .detail-body { color: var(--text-secondary); line-height: 1.8; white-space: pre-wrap; }
    ...
    .compose-modal { background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 16px; width: 100%; max-width: 600px; }
    .compose-header { display: flex; justify-content: space-between; align-items: center; padding: 1.2rem 1.5rem; border-bottom: 1px solid var(--border-color); }
    .compose-header h2 { margin: 0; color: var(--text-primary); font-size: 1.1rem; }
    .compose-field { padding: 0 1.5rem; }
    .compose-field:first-of-type { padding-top: 1.2rem; }
    .compose-field label { display: block; color: var(--text-secondary); font-size: 0.8rem; padding: 0.5rem 0 0.3rem; }
    .compose-field input, .compose-field textarea { width: 100%; background: transparent; border: none; border-bottom: 1px solid var(--border-color); color: var(--text-primary); padding: 0.6rem 0; font-size: 0.95rem; outline: none; font-family: inherit; resize: none; }
    .compose-field input:focus, .compose-field textarea:focus { border-color: var(--primary-color); }
    .compose-field textarea { border: 1px solid var(--border-color); border-radius: 8px; margin: 0.5rem 0; padding: 0.8rem; min-height: 200px; }
    .compose-actions { display: flex; gap: 0.8rem; justify-content: flex-end; padding: 1rem 1.5rem; border-top: 1px solid var(--border-color); }
